...
function ExpressProductList(props) {
    ...
    const receivedData = async (val) => {
        ...
        if (props.match.params.hasOwnProperty("vendorId")) {
            urlString = props.match.params.vendorId === "MWzJ2s6kM5ZUZyaa4l2o37ZQCWj2"
                ? "vendor-product-m/" + "GHS5sVHoRShSE2KmLtvVCGue8X82" + "/express/query?size=" + size + "&page=" + val
                : "vendor-product-m/" + props.match.params.vendorId + "/express/query?size=" + size + "&page=" + val;
        }
    ...
    return (
        <div>
            ...
            <center><h2 style={{ marginTop: -9, marginBottom: 0, fontStyle: 'italic', color: 'white' }}>Express Products</h2></center>
